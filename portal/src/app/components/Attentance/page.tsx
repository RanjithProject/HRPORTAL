    ...
    loginHistory.forEach((entry, index) => {
      if (entry.type === 'break' && entry.action === 'Break Out') {
        ...
        if (breakInEntry) {
          ...
          const breakDurationMilliseconds = breakDuration % 1000;

          breakEntries.push({
            breakIn: breakIn.toLocaleString(),
            breakOut: breakOut.toLocaleString(),
            breakDuration: `${breakDurationHours} hours ${breakDurationMinutes} minutes ${breakDurationSeconds} seconds ${formatMilliseconds(breakDurationMilliseconds)} ms`
          });
        }
      }
    });
      ...
      if (entry.action === 'Sign In' || entry.action === 'Sign Out') {
        ...
        if (entry.action === 'Sign In') {
          ...
          if (lastSignIn) {
            ...
            lastSignIn.duration = `${formatTime(duration.hours)}:${formatTime(duration.minutes)}:${formatTime(duration.seconds)}:${formatMilliseconds(duration.milliseconds)} ms`;
          }
        }
      }
    });
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="mt-6">
        ...
        <p>
          ...
          {formatMilliseconds(totalBreakTime % 1000)} ms
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        <p>
          ...
          {formatMilliseconds(totalWorkingTime % 1000)} ms
        </p>
      </div>
    </div>
  );
